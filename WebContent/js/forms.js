 ...
 * @version $Id: koLibrary-1.0.0.25e-1-beta1-js1$
 */
...
(function(){
	Forms.Utils={
			...
			setInnerHTML:function(divContent, HTML,change) {
				if(Object.isString(divContent))
					divContent=$(divContent);
				if(divContent!=null){
...
(function(){
	...
	Forms.Validator.prototype={
		...
		getElementsToControl:function(){
			...
			var getAllElementsOfaForm=function(aForm){
				var ret=new Array();
				if(aForm){
			...
			var elementsToControl=new Array();
			if(this.id && $(this.id)){
				elementsToControl=getAllElementsOfaForm($(this.id));
			}else
				elementsToControl=getAllElementsOfAllForms(document.forms);
			return elementsToControl;
		},
		addElementsToControl: function(elements){
			...
			if(elements!=undefined){
				if(typeof(elements)=='string'){
					...
					this.elementsToControl=this.getElementsToControl();
			}else
				this.elementsToControl=this.getElementsToControl();
		},
		validate:function (){
			var valide=true;
			var result=Forms.Utils.dispatchCustomEvent(this.id, "validate", {"form":$(this.id),"validator":this});
			if(result){
				for(var i=0;i<this.elementsToControl.length;i++){
					try {
						if (!this.scontrol(this.elementsToControl[i],true)) {
							valide=false;
							}
					}
					catch(ex){}
				...
				if(!valide){
					...
					Forms.Utils.fireEvent($(this.id), "validateerror");
				}else
					Forms.Utils.fireEvent($(this.id), "validatesuccess");
				this.onInit=false;
				result=Forms.Utils.dispatchCustomEvent(this.id, "validateend", {"form":$(this.id),"validator":this,"success":valide});
	...
	//--------------------------------------------------------------------
		scontrol:function(pMyObject,fromValidate){
			try{
			...
			var result=true;
			if(isInvalidControl){
				result=Forms.Utils.dispatchCustomEvent(pMyObject, "validatefielderror", {"form":$(this.id),"validator":this,"field":pMyObject});
				if(!fromValidate)
					Forms.Utils.fireEvent($(this.id), "validateerror");
			}
			else
				result=Forms.Utils.dispatchCustomEvent(pMyObject, "validatefieldsuccess", {"form":$(this.id),"validator":this,"field":pMyObject});
			if(!fromValidate)
				Forms.Utils.dispatchCustomEvent(this.id, "validateend", {"form":$(this.id),"validator":this,"success":!isInvalidControl});
			if(result){
	...
	Forms.Ajax.prototype={
		...
		load:function(args){
					...
					if(xObj.readyState  == 4){
						...
						if(xObj.status  == 200){
							...
							if(_this.id && document.getElementById(_this.id)!=undefined)
								...
								if(!_this.isUpload || _this.uploadEnded){
									if (_this.func!=null) {
			...
			if(!this.async){
				...
				var i=requests.indexOf(this.uid);
				if(i>-1)
					requests.splice(i, 1);
			}
		},
		...
		initUpload:function(xhrequest){
			if(supportAjaxUploadWithProgress()){
				...
				if(this.func)
					xhrequest.upload.addEventListener('loadend', function(){_this.uploadEnded=true;}, false);
			}
		}
	};
	//Membres privés
...
(function(){
	...
	Forms.Elements.prototype={
			...
			getAttrs:function(attr,separator){
				var resultArray=new Array();
				values=this.values;
				for(var i=0;i<values.length;i++){
					if(values[i][attr])
						resultArray.push(values[i][attr]);
				}
				if(separator)
